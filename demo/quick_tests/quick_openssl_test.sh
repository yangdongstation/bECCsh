...
if [[ ! -f "$BECCSH_DIR/pure_bash_loader.sh" ]]; then
    echo "❌ bECCsh模块未找到: $BECCSH_DIR/pure_bash_loader.sh"
    exit 1
fi
...
echo "  Gx: 55066263022277343669578718895168534326250603453777594175500187360389116729240"
echo "  Gy: 32670510020758816978083085130507043184471273380659243275938904335757337482424"

echo "OpenSSL secp256k1参数:"
openssl ecparam -name secp256k1 -text -noout 2>/dev/null | grep -A 3 "Field Type:" | head -5
echo "  （参数与bECCsh一致）"
